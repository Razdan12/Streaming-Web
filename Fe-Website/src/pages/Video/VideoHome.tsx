...
import { SliderBannerVideo } from "../../component/Carousel";
...
const VideoHome = () => {
  ...
  const { setIdVideo, setNamaFile, setKategori, setNamaKategori } = useStore();
  ...
  const fetchSearchData = async (param: string) => {
    try {
      setLoading(true);
      const responseSearch = await api.searchVideo(param);
      setVideoAll(responseSearch.data);
    } catch (error) {
      console.log(error);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    search ? fetchSearchData(search) : fetchVideoByKategori();
    videoByKategori ? setLoading(false) : setLoading(true)
  }, [triger]);
          ...
          <div className="w-full">
            ...
            <div className="flex w-full justify-center items-center flex-col  mt-5">
              <div className="w-full  sm:w-5/6 ">
                <p className="text-3xl px-4 font-bold">Video</p>
                        ...
                        <div className=" w-full px-8 sm:p-2">
                          <Slider {...settings}>
                            {item.video.map((video: any, index: number) => (
                              <div key={index}>
                                <div className="w-36   sm:w-42 md:w-62 lg:w-60 xl:w-60 2xl:w-80">
                                  <img
                                    id={video.id}
                                    src={video.thumbnail}
                                    alt={video.judul}
                    ...
                    {videoAll?.data?.map((item: any, number: number) => (
                      <div className="w-40 sm:w-72 cursor-pointer" key={number}>
                        <img
                          ...
                          onClick={() =>
                            handlePlayVideo(item?.id, item?.namaFIle)
                          }
                        />
                      </div>
                    ))}
